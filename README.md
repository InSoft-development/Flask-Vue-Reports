...
1. скачать релизную версию из репозитория;
...
3. перейти в директорию с веб-приложением и создать структуру веб-приложения с помощью команды:

```
python app.py -s
```

4. в директории веб-приложения `./client` дать права на чтение, запись и исполнение бинарному файлу `client` и скрипту `slicer.py`:

```
chmod 775 client slicer.py
```

5. в директорию веб-приложения `./data` положить сертификаты `cert.pem`, `key.perm` для обеспечения шифрованной передачи данных между сервером и клиенту по протоколу https. Если сертификаты отсутствуют, то их следует сгенерировать с помощью пакета openssl. 
...
6. в директорию веб-приложения `./data` скопировать при наличии файл тега `kks_all.csv`. Затем в той же директории `./data` создать копию файла `kks_all.csv` и назвать ее `kks_all_back.csv`;
7. в директорию веб-приложения `./data` скопировать при наличии конфиг `config.json`.  Если конфига нет, то создать его по описанию из раздела: [[#Описание конфигурационного файла config.json]].
